...
function pathSet(path,val){ pathSetRaw(path,val); syncCaseAfterPath(path); scheduleSave(); }
...
function bindInputs(){
  content.querySelectorAll('[data-path]').forEach(el=>el.addEventListener('input', e=>{ pathSet(e.target.dataset.path, e.target.value); if(e.target.dataset.path.endsWith('.interviewType')) render(current); }));
...
function evidenceTemplate(){return {id:id(),number:nextNumber(state.evidence,state.settings.evidencePrefix || 'E'),propertyCounter:'No',description:'',location:'',collectedBy:lastEvidenceCollector(),date:new Date().toISOString().slice(0,10),packaging:'',station:'',locker:'',lab:'',notes:''};}
