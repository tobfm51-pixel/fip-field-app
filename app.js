...
const blank = () => {
  ...
  sectionStatus:Object.fromEntries(SCREEN_REGISTRY.map(screen=>[screen.id,'Not Started'])),
...
function migrate(data){
  ...
  if(!out.sectionStatus || typeof out.sectionStatus !== 'object' || Array.isArray(out.sectionStatus)) out.sectionStatus = {};
  SCREEN_REGISTRY.forEach(screen=>{ if(out.sectionStatus[screen.id] === undefined) out.sectionStatus[screen.id] = 'Not Started'; });
...
function render(screen=current){
  ...
  updateCaseRibbon();
  const map={dashboard,initial,scene,building,deck,utilities,electrical,exterior,roof,interior,rooms,areaOrigin,firePatterns,ignitionSources,ignitionMatrix,people,vehicles,machinery,exposures,photos,evidence,interviews,smokeAlarms,timeline,tasks,reports,files};
...
function dashboard(){
  ...
  const statusValues = Object.values(state.sectionStatus || {});
  const completeCount = statusValues.filter(v=>v==='Complete' || v==='N/A').length;
  const totalSections = SCREEN_REGISTRY.length;
  const progress = totalSections ? Math.round((completeCount / totalSections) * 100) : 0;
  const health = progress >= 80 ? 'Good' : progress >= 40 ? 'In Progress' : 'Needs Attention';
  const completionRows = SCREEN_REGISTRY.map(screen=>`<div class="item"><div class="item-head"><strong>${esc(screen.title)}</strong><span class="badge">${esc(pathGet(`sectionStatus.${screen.id}`) || 'Not Started')}</span></div>${field('Completion Status',`sectionStatus.${screen.id}`,'select',{options:['Not Started','Started','Complete','N/A']})}</div>`).join('');
...
function evidenceTemplate(){return {id:id(),number:String((state.evidence?.length||0)+1).padStart(3,'0'),propertyCounter:'No',description:'',location:'',collectedBy:lastEvidenceCollector(),date:new Date().toISOString().slice(0,10),packaging:'',station:'',locker:'',lab:'',notes:''};}
...
function interviewPromptBlock(type){ const prompts = INTERVIEW_PROMPTS[type] || INTERVIEW_PROMPTS.Other; return `<div class="warn"><strong>Dynamic prompts for ${esc(type || 'Other')}:</strong><ul>${prompts.map(q=>`<li>${esc(q)}</li>`).join('')}</ul></div>`; }
function interviewTemplate(guide='Owner'){return {id:id(),guide,interviewType:guide,personId:'',person:'',date:new Date().toISOString().slice(0,10),time:'',location:'',questions:[],summary:'',notes:'',followUp:''};}
...
function evidenceCard(x,i,k){ return commonItem(x,i,k,`Evidence ${x.number||i+1}`,`${dynField(k,i,'number','Evidence #')}${dynField(k,i,'description','Description','textarea')}${dynField(k,i,'location','Location Collected')}${dynField(k,i,'collectedBy','Collected By')}${dynField(k,i,'date','Date Collected','date')}${dynField(k,i,'propertyCounter','Property Counter','select',{options:['No','Yes']})}${dynField(k,i,'packaging','Packaging')}${dynField(k,i,'station','Station')}${dynField(k,i,'locker','Locker')}${dynField(k,i,'lab','Lab / Submission')}${dynField(k,i,'notes','Notes','textarea')}`); }
function interviewCard(x,i,k){ const linked=(state.people||[]).find(p=>p.id===x.personId); const type=x.interviewType || x.guide || 'Other'; return commonItem(x,i,k,`${type}: ${linked?.name || x.person || 'Unlinked person'}`,`${personSelect('Linked Person',`${k}.${i}.personId`)}${dynField(k,i,'interviewType','Interview Type','select',{options:INTERVIEW_TYPES})}${interviewPromptBlock(type)}${dynField(k,i,'person','Person Interviewed (free text override)')}${dynField(k,i,'date','Date','date')}${dynField(k,i,'time','Time','time')}${dynField(k,i,'location','Location')}${dynField(k,i,'summary','Summary','textarea')}${dynField(k,i,'notes','Free-form Notes','textarea')}${dynField(k,i,'followUp','Follow-up Needed','textarea')}`); }
function handleAction(e){ const a=e.currentTarget.dataset.action, key=e.currentTarget.dataset.key; if(a==='goDashboard') render('dashboard'); if(a==='add'){ collectionAdd(key,templates[key]()); render(current); } if(a==='remove'){ collectionRemove(key,e.currentTarget.dataset.index); render(current); } if(a==='addInterview'){ state.interviews.push(interviewTemplate(e.currentTarget.dataset.guide)); save(); render('interviews'); } if(a==='exportFip') exportFip(false); if(a==='shareFip') exportFip(true); if(a==='importFip') $('#importFile').click(); if(a==='newCase') { if(confirm('Create a new blank case? Export the current case first if needed.')) caseEngine.create(); } if(a==='copyReport'){ navigator.clipboard?.writeText(reportText()); saveStatus.textContent='Report copied'; } if(a==='exportSummaryReport') exportText(`${filename().replace('.fip','')}_summary.txt`, reportText()); if(a==='exportInitialReport') exportText(`${filename().replace('.fip','')}_initial.txt`, initialReportText()); if(a==='exportCrispNotes') exportText(`${filename().replace('.fip','')}_crisp_notes.txt`, crispNotesText());}
...
function crispNotesText(){ return [`Case: ${state.initial.caseNumber || ''}`,`Address: ${state.initial.incidentAddress || ''}`,`Area of Origin: ${state.report.areaOfOrigin || ''}`,`Cause: ${state.report.causeClassification || state.initial.cause || ''}`,`Ignition Source: ${state.report.ignitionSource || ''}`,`First Fuel: ${state.report.firstFuel || ''}`,`Status: ${state.report.status || ''}`,state.report.narrative || ''].join('\n'); }
