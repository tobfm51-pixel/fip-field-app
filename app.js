...
function render(screen=current){
  ...
  const map={dashboard,newCaseWizard,initial,scene,building,deck,utilities,electrical,exterior,roof,interior,vehicles,machinery,exposures,rooms,areaOrigin,firePatterns,ignitionSources,ignitionMatrix,people,investigationItems,photos,evidence,interviews,smokeAlarms,timeline,tasks,reports,files};
...
function investigationItemLines(){ return (state.investigationItems||[]).map(item=>`${item.itemType || 'Item'}: ${[item.applianceType || item.description, item.manufacturer, item.modelNumber, item.serialNumber, item.location, item.notes].filter(Boolean).join(' • ')}`).filter(line=>!line.endsWith(': ')); }
function crispNotesText(){ const evidenceLines=evidenceDispositionLines(); const itemLines=investigationItemLines(); return [`Case: ${state.initial.caseNumber || ''}`,`Address: ${state.initial.incidentAddress || ''}`,`Area of Origin: ${state.report.areaOfOrigin || ''}`,`Cause: ${state.report.causeClassification || state.initial.cause || ''}`,`Ignition Source: ${state.report.ignitionSource || ''}`,`First Fuel: ${state.report.firstFuel || ''}`,`Status: ${state.report.status || ''}`, evidenceLines.length ? `Evidence Secured:\n${evidenceLines.join('\n')}` : '', itemLines.length ? `Additional Investigation Items:\n${itemLines.join('\n')}` : '', state.report.narrative || ''].filter(Boolean).join('\n'); }
function initialReportText(){ const i=state.initial; const itemLines=investigationItemLines(); return [`Initial Report`, `Case#: ${i.caseNumber || ''}`, `Incident Type: ${i.incidentType || ''}`, `Address: ${i.incidentAddress || ''}`, `Reported: ${i.reportedDate || ''} ${i.reportedTime || ''}`, `Dispatch: ${i.dispatchDate || ''} ${i.dispatchTime || ''}`, `Arrival: ${i.arrivalDate || ''} ${i.arrivalTime || ''}`, `Investigator: ${state.settings.investigator || ''}`, `Initial Notes: ${i.notes || ''}`, itemLines.length ? `Additional Investigation Items:\n${itemLines.join('\n')}` : ''].filter(Boolean).join('\n'); }
