...
function investigationItemsScreen(){
  ...
  const add = card('Additional Investigation Items',`<p class="muted">Add vehicles, machinery/equipment, exposure structures, batteries, injuries/fatalities, and other items for follow-up documentation. Appliance is listed for planning but its form is not built yet.</p><div class="field"><label>Item Type</label><select data-path="ui.newInvestigationItemType">${typeOptions}</select></div><div class="actions"><button class="btn blue" data-action="addInvestigationItem">+ Add Item</button></div>`);
...
function investigationItemCard(item,i){
  ...
  if(item.itemType==='Appliance') return `<div class="item"><div class="item-head"><strong>${esc(head)}</strong>${remove}</div><div class="warn">Appliance investigation item form is not built yet.</div></div>`;
