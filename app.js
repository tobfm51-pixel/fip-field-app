...
function migrate(data){
  ...
  ['people','vehicles','machinery','rooms','windows','originAreas','firePatterns','ignitionSources','ignitionMatrix','exposureStructures','evidence','photos','interviews','notes','assignments'].forEach(k=>{ if(!Array.isArray(out[k])) out[k]=[]; });
...
function render(screen=current){
  ...
  const map={dashboard,initial,scene,building,utilities,electrical,exterior,roof,interior,rooms,areaOrigin,firePatterns,ignitionSources,ignitionMatrix,people,vehicles,machinery,exposures,photos,evidence,interviews,reports,files};
