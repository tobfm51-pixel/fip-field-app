...
function bindInputs(){
  content.querySelectorAll('[data-path]').forEach(el=>{ const handler=e=>{ pathSet(e.target.dataset.path, e.target.value); if(e.target.dataset.path.endsWith('.interviewType') || e.target.dataset.path.endsWith('.evidenceSecured')) render(current); }; el.addEventListener(el.tagName==='SELECT' ? 'change' : 'input', handler); });
