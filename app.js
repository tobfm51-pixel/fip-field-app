...
function visibleScreens(){
  ...
  const base = new Set(['dashboard','initial','scene','people','investigationItems','photos','evidence','interviews','timeline','tasks','reports','files','areaOrigin','firePatterns','ignitionSources','ignitionMatrix']);
...
function render(screen=current){
  ...
  const map={dashboard,newCaseWizard,initial,scene,building,deck,utilities,electrical,exterior,roof,interior,rooms,areaOrigin,firePatterns,ignitionSources,ignitionMatrix,people,investigationItems,photos,evidence,interviews,smokeAlarms,timeline,tasks,reports,files};
...
const INVESTIGATION_ITEM_TYPES = ['Vehicle','Machinery / Equipment','Exposure Structure','Appliance','Battery / Energy Storage','Injury / Fatality','Other Item'];
const APPLIANCE_TYPES = ['Dryer','Washer','Range / Oven','Cooktop','Microwave','Refrigerator','Freezer','Dishwasher','Coffee Maker','Toaster / Toaster Oven','Space Heater','Fireplace / Insert','Pellet Stove','HVAC / Furnace','Water Heater','Battery Charger','Other'];
...
function investigationItemTemplate(type='Other Item'){
  ...
  if(type==='Appliance') Object.assign(item, applianceTemplateFields());
  return item;
}
function applianceTemplateFields(){ return {applianceType:'Dryer',manufacturer:'',modelNumber:'',serialNumber:'',dateCode:'',powerSource:'',location:'',pluggedIn:'Unknown',energized:'Unknown',operatingAtTimeOfFire:'Unknown',lastKnownUse:'',conditionObserved:'',fireDamageObserved:'',electricalDamageObserved:'',recallCheckNeeded:'Yes',recallChecked:'No',recallSource:'',recallResults:'',notes:'',recallReminderShown:false}; }
...
function investigationItemCard(item,i){
  ...
  if(item.itemType==='Appliance'){ Object.assign(item, {...applianceTemplateFields(), ...item}); return `<div class="item"><div class="item-head"><strong>${esc(head)}</strong>${remove}</div>${applianceFields('investigationItems',i)}<div class="actions"><button class="btn secondary" data-action="checkRecalls" data-index="${i}">Check Recalls</button></div>${item.recallReminderShown?'<div class="warn">Reminder: check CPSC and manufacturer recalls, then document the source and results.</div>':''}</div>`; }
...
function applianceFields(k,i){ return `${dynField(k,i,'applianceType','Appliance Type','select',{options:APPLIANCE_TYPES})}${dynField(k,i,'manufacturer','Manufacturer')}${dynField(k,i,'modelNumber','Model Number')}${dynField(k,i,'serialNumber','Serial Number')}${dynField(k,i,'dateCode','Date Code')}${dynField(k,i,'powerSource','Power Source')}${dynField(k,i,'location','Location')}${dynField(k,i,'pluggedIn','Plugged In','select',{options:YES_NO_UNKNOWN})}${dynField(k,i,'energized','Energized','select',{options:YES_NO_UNKNOWN})}${dynField(k,i,'operatingAtTimeOfFire','Operating at Time of Fire','select',{options:YES_NO_UNKNOWN})}${dynField(k,i,'lastKnownUse','Last Known Use')}${dynField(k,i,'conditionObserved','Condition Observed','textarea')}${dynField(k,i,'fireDamageObserved','Fire Damage Observed','textarea')}${dynField(k,i,'electricalDamageObserved','Electrical Damage Observed','textarea')}${dynField(k,i,'recallCheckNeeded','Recall Check Needed','select',{options:YES_NO})}${dynField(k,i,'recallChecked','Recall Checked','select',{options:YES_NO})}${dynField(k,i,'recallSource','Recall Source')}${dynField(k,i,'recallResults','Recall Results','textarea')}${dynField(k,i,'notes','Notes','textarea')}`; }
...
function handleAction(e){ const a=e.currentTarget.dataset.action, key=e.currentTarget.dataset.key; if(a==='goDashboard') render('dashboard'); if(a==='addInvestigationItem'){ addInvestigationItem(pathGet('ui.newInvestigationItemType') || 'Vehicle'); render('investigationItems'); } if(a==='add'){ collectionAdd(key,templates[key]()); render(current); } if(a==='removeInvestigationItem'){ removeInvestigationItem(e.currentTarget.dataset.index); render('investigationItems'); } if(a==='checkRecalls'){ const item=state.investigationItems[Number(e.currentTarget.dataset.index)]; if(item){ item.recallReminderShown=true; saveStatus.textContent='Reminder: check CPSC and manufacturer recalls and document findings.'; save(); render('investigationItems'); } } if(a==='remove'){ collectionRemove(key,e.currentTarget.dataset.index); render(current); } if(a==='addInterview'){ state.interviews.push(interviewTemplate(e.currentTarget.dataset.guide)); save(); render('interviews'); } if(a==='exportFip') exportFip(false); if(a==='shareFip') exportFip(true); if(a==='importFip') $('#importFile').click(); if(a==='newCaseWizard') render('newCaseWizard'); if(a==='createCaseFromWizard') caseEngine.createFromWizard(); if(a==='copyReport'){ navigator.clipboard?.writeText(reportText()); saveStatus.textContent='Report copied'; } if(a==='exportSummaryReport') exportText(`${filename().replace('.fip','')}_summary.txt`, reportText()); if(a==='exportInitialReport') exportText(`${filename().replace('.fip','')}_initial.txt`, initialReportText()); if(a==='exportCrispNotes') exportText(`${filename().replace('.fip','')}_crisp_notes.txt`, crispNotesText());}
