...
const blank = () => {
  ...
  utilities:{electricOn:'',electricServiceType:'',electricProvider:'',electricProviderOther:'',electricMeterLocation:'',mainDisconnectLocation:'',gasOn:'',fuelType:'',gasProvider:'',gasMeterLocation:'',lpTank:'',lpTankSize:'',lpLocation:'',waterOn:'',waterProvider:'',waterShutoffLocation:'',notes:''},
...
function utilities(){ setTitle('Utilities'); return `<div class="grid">${card('Electric Utility Service',`${seg('Electricity','utilities.electricOn',['On','Off','None'])}${seg('Electric Service Type','utilities.electricServiceType',['Overhead','Service Lateral','Underground','None'])}${field('Location of Electric Meter','utilities.electricMeterLocation')}${field('Main Disconnect Location','utilities.mainDisconnectLocation')}${field('Electric Service Provider','utilities.electricProvider','select',{options:['','Dominion Power','NOVEC','Virginia Power','Other']})}${field('Other Electric Provider','utilities.electricProviderOther')}`)}${card('Gas / Fuel Utility Service',`${seg('Gas/Fuel','utilities.gasOn',['On','Off','None'])}${seg('Fuel Type','utilities.fuelType',['Natural Gas','Propane','Oil','Other'])}${field('Gas Provider','utilities.gasProvider')}${field('Gas Meter Location','utilities.gasMeterLocation')}${seg('LP Tank Present','utilities.lpTank',['Yes','No','Unknown'])}${field('LP Tank Size','utilities.lpTankSize')}${field('LP Tank Location','utilities.lpLocation')}`)}${card('Water / Other Utilities',`${seg('Water Service','utilities.waterOn',['On','Off','Unknown'])}${field('Water Provider','utilities.waterProvider')}${field('Water Shutoff Location','utilities.waterShutoffLocation')}${field('Utility Notes','utilities.notes','textarea')}`)}</div>`; }
