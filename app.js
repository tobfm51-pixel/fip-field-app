...
const MODULE_VISIBILITY = {
  Structure:['structure','exposure'],
  Vehicle:['vehicle','exposure'],
  Machinery:['machinery','exposure'],
  'Outside Fire':['exposure'],
  Explosion:['structure','vehicle','machinery','exposure'],
  Other:['structure','vehicle','machinery','exposure']
};
...
const blank = () => {
  ...
  people:[], vehicles:[], machinery:[], rooms:[], windows:[], decks:[], smokeAlarms:[], originAreas:[], firePatterns:[], ignitionSources:[], ignitionMatrix:[], exposureStructures:[], evidence:[], photos:[], timelineEvents:[], interviews:[], notes:[], assignments:[],
...
function migrate(data){
  ...
  ['people','vehicles','machinery','rooms','windows','decks','smokeAlarms','originAreas','firePatterns','ignitionSources','ignitionMatrix','exposureStructures','evidence','photos','timelineEvents','interviews','notes','assignments'].forEach(k=>{ if(!Array.isArray(out[k])) out[k]=[]; });
...
function applyIncidentModuleVisibility(){
  const visible = MODULE_VISIBILITY[state.case?.incidentType || state.initial?.incidentType || 'Structure'] || MODULE_VISIBILITY.Other;
  document.querySelectorAll('[data-incident-module]').forEach(btn=>{
    const show = visible.includes(btn.dataset.incidentModule);
    btn.classList.toggle('module-hidden', !show);
    btn.disabled = !show;
  });
}
function bindInputs(){
...
function render(screen=current){
  ...
  const map={dashboard,initial,scene,building,utilities,electrical,exterior,roof,interior,rooms,smokeAlarms,decks,areaOrigin,firePatterns,ignitionSources,ignitionMatrix,people,vehicles,machinery,exposures,photos,timeline,evidence,interviews,reports,files};
  content.innerHTML = requested !== 'dashboard' && !hasActiveCase() ? noActiveCaseScreen() : (map[requested]||dashboard)();
  bindInputs();
  applyIncidentModuleVisibility();
}
...
function dashboard(){
  ...
  if(!hasActiveCase()){
    return `<div class="grid full">${card('New Case Wizard',`<div class="warn">No case is active. Create a new case or open a .fip file to begin.</div><div class="actions"><button class="btn secondary" data-action="newCase">New Case Wizard</button><button class="btn secondary" data-action="importFip">Open .fip</button></div>`)}</div>`;
  }
...
function smokeAlarms(){ setTitle('Smoke Alarm Module'); return `<div class="grid full">${card('Smoke Alarm Summary',`${seg('Smoke Alarms Present','lifeSafety.smokeAlarms',['Yes','No','Unknown'])}${seg('Alerted Occupants','lifeSafety.smokeAlerted',['Yes','No','Unknown'])}${field('Smoke Alarm Notes','building.alarmNotes','textarea')}`)}${dynamicInner('smokeAlarms','Smoke Alarm Devices',smokeAlarmTemplate,smokeAlarmCard)}</div>`; }
function decks(){ setTitle('Decks'); return dynamicScreen('decks','Decks / Porches',deckTemplate,deckCard); }
function timeline(){ setTitle('Timeline'); return dynamicScreen('timelineEvents','Timeline Events',timelineEventTemplate,timelineEventCard); }
...
function deckTemplate(){return {id:id(),location:'',material:'',attachment:'',condition:'',damage:'',notes:''};}
function smokeAlarmTemplate(){return {id:id(),location:'',type:'',power:'',operated:'Unknown',condition:'',notes:''};}
function timelineEventTemplate(){return {id:id(),date:new Date().toISOString().slice(0,10),time:'',type:'Investigation Event',description:'',source:''};}
...
function evidenceTemplate(){return {id:id(),number:String((state.evidence?.length||0)+1).padStart(3,'0'),description:'',location:'',collectedBy:state.settings.investigator,date:new Date().toISOString().slice(0,10),packaging:'',secured:'No',securedBy:'',securedDate:'',securedTime:'',storageLocation:'',lab:'',chainOfCustody:'',notes:''};}
...
function deckCard(x,i,k){ return commonItem(x,i,k,`${x.location||'Deck / Porch'}`,`${dynField(k,i,'location','Location')}${dynField(k,i,'material','Deck Material')}${dynField(k,i,'attachment','Attachment / Support')}${dynField(k,i,'condition','Condition','textarea')}${dynField(k,i,'damage','Fire Damage','textarea')}${dynField(k,i,'notes','Notes','textarea')}`); }
function smokeAlarmCard(x,i,k){ return commonItem(x,i,k,`${x.location||'Smoke Alarm'}`,`${dynField(k,i,'location','Location')}${dynField(k,i,'type','Type','select',{options:['','Ionization','Photoelectric','Combination','CO/Smoke','Unknown']})}${dynField(k,i,'power','Power Source','select',{options:['','Hardwired','Battery','Hardwired with Battery Backup','Unknown']})}${dynField(k,i,'operated','Operated?','select',{options:['Yes','No','Unknown']})}${dynField(k,i,'condition','Condition / Findings','textarea')}${dynField(k,i,'notes','Notes','textarea')}`); }
function timelineEventCard(x,i,k){ return commonItem(x,i,k,`${x.date||'Date'} ${x.time||''} - ${x.type||'Timeline Event'}`,`${dynField(k,i,'date','Date','date')}${dynField(k,i,'time','Time','time')}${dynField(k,i,'type','Event Type','select',{options:['Dispatch','Arrival','Fire Event','Investigation Event','Evidence Event','Interview Event','Scene Released','Other']})}${dynField(k,i,'description','Description','textarea')}${dynField(k,i,'source','Source / Reference')}`); }
...
function peopleOptions(){ return ['', state.settings.investigator, ...(state.people||[]).map(p=>p.name).filter(Boolean)]; }
function evidenceCard(x,i,k){ return commonItem(x,i,k,`Evidence ${x.number||i+1}`,`${dynField(k,i,'number','Evidence #')}${dynField(k,i,'description','Description','textarea')}${dynField(k,i,'location','Location Collected')}${dynField(k,i,'collectedBy','Collected By','select',{options:peopleOptions()})}${dynField(k,i,'date','Date Collected','date')}${dynField(k,i,'packaging','Packaging')}${dynField(k,i,'secured','Evidence Secured?','select',{options:['No','Yes','Transferred']})}${dynField(k,i,'securedBy','Secured By','select',{options:peopleOptions()})}${dynField(k,i,'securedDate','Secured Date','date')}${dynField(k,i,'securedTime','Secured Time','time')}${dynField(k,i,'storageLocation','Storage / Locker Location')}${dynField(k,i,'lab','Lab / Submission')}${dynField(k,i,'chainOfCustody','Chain of Custody Notes','textarea')}${dynField(k,i,'notes','Notes','textarea')}`); }
const INTERVIEW_PROMPTS = {
  'Firefighter Observation':['Arrival conditions','Fire/smoke location','Suppression actions','Ventilation/overhaul observations'],
  Owner:['Occupancy status','Last known normal condition','Utilities/appliances in use','Insurance or prior issues'],
  Employee:['Work activity before incident','Equipment used','Closing/opening procedure','Unusual odors/sounds'],
  Contractor:['Scope of work','Tools/hot work used','Materials stored','Last time on scene'],
  'Discoverer / First Witness':['How fire was discovered','First observed location','Actions taken','Who was notified'],
  'Neighborhood Canvass':['What was seen/heard','Time observed','Camera availability','Prior suspicious activity'],
  'Case Solvability':['Known witnesses','Video/evidence leads','Timeline gaps','Follow-up priority'],
  General:['Who/what/when/where','Sequence of events','Observations','Follow-up leads']
};
function interviewPrompts(guide){ return `<div class="warn"><strong>Prompts:</strong><ul>${(INTERVIEW_PROMPTS[guide]||INTERVIEW_PROMPTS.General).map(p=>`<li>${esc(p)}</li>`).join('')}</ul></div>`; }
function interviewCard(x,i,k){ return commonItem(x,i,k,`${x.guide||'Interview'}: ${x.person||'Unnamed'}`,`${dynField(k,i,'guide','Guide / Type','select',{options:['General','Firefighter Observation','Owner','Employee','Contractor','Discoverer / First Witness','Neighborhood Canvass','Case Solvability']})}${interviewPrompts(x.guide||'General')}${dynField(k,i,'person','Person Interviewed','select',{options:peopleOptions()})}${dynField(k,i,'date','Date','date')}${dynField(k,i,'time','Time','time')}${dynField(k,i,'location','Location')}${dynField(k,i,'summary','Summary','textarea')}${dynField(k,i,'followUp','Follow-up Needed','textarea')}`); }
