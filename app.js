...
const blank = () => {
  ...
  case:{id:caseId,active:true,caseNumber:'',incidentType:'Structure',incidentAddress:'',jurisdiction:'',primaryInvestigator:'C. Mount, #5572',assistedBy:'',caseStatus:'Draft',createdAt:now,createdBy:'',updatedAt:now,updatedBy:''},
  initial:{caseNumber:'',incidentNumber:'',incidentType:'Structure',cause:'Undetermined',dateNotified:'',timeNotified:'',reportedDate:'',reportedTime:'',dispatchSame:'',dispatchDate:'',dispatchTime:'',arrivalDate:'',arrivalTime:'',sceneReleasedDate:'',sceneReleasedTime:'',incidentAddress:'',city:'',state:'VA',jurisdiction:'',authority:'Exigency',consentName:'',otherLEO:[],leoOther:'',notes:''},
  ...
  utilities:{electricOn:'',electricServiceType:'',electricProvider:'',electricProviderOther:'',electricMeterLocation:'',electricMeterNumber:'',electricMeterDamage:'',electricNotes:'',gasOn:'',fuelType:'',naturalGasPresent:'',gasProvider:'',gasMeterLocation:'',gasMeterNumber:'',gasRiserLocation:'',gasMeterDamage:'',gasRiserDamage:'',gasNotes:'',lpSupplier:'',lpTank:'',lpTankSize:'',lpLocation:'',lpOrientation:'',lpSerialNumber:'',lpPercentProduct:'',lpRegulators:'',lpTankDamage:'',lpNotes:'',generator:'',batteryStorage:'',energySystemNotes:'',notes:''},
  electrical:{panels:[]},
...
function migrateElectrical(target){
  if(!target.electrical || typeof target.electrical !== 'object' || Array.isArray(target.electrical)) target.electrical = {panels:[]};
  if(!Array.isArray(target.electrical.panels)) target.electrical.panels = [];
  target.electrical.panels = target.electrical.panels.map(panel => ({...electricalPanelTemplate(), ...panel, circuits:Array.isArray(panel?.circuits) ? panel.circuits.map(circuit => ({...circuitTemplate(), ...circuit})) : []}));
}
...
function render(screen=current){
  ...
  const map={dashboard,initial,scene,building,exterior,roof,interior,utilities,electrical,people,vehicles,machinery,rooms,photos,evidence,interviews,reports,files};
...
function utilities(){
  setTitle('Utilities');
  return `<div class="grid">${card('Electric Service',`${field('Electric Provider','utilities.electricProvider','select',{options:['','Dominion Power','NOVEC','Virginia Power','Other']})}${field('Other Electric Provider','utilities.electricProviderOther')}${seg('Electric Service Type','utilities.electricServiceType',['Overhead','Service Lateral','Underground','None','Unknown'])}${field('Electric Meter Location','utilities.electricMeterLocation')}${field('Electric Meter Number','utilities.electricMeterNumber')}${seg('Fire Damage at Electric Meter','utilities.electricMeterDamage',['Yes','No','Unknown'])}${field('Electric Notes','utilities.electricNotes','textarea')}`)}${card('Natural Gas',`${seg('Natural Gas Present','utilities.naturalGasPresent',['Yes','No','Unknown'])}${field('Gas Provider','utilities.gasProvider')}${field('Gas Meter Location','utilities.gasMeterLocation')}${field('Gas Meter Number','utilities.gasMeterNumber')}${field('Gas Riser Location','utilities.gasRiserLocation')}${seg('Damage at Gas Meter','utilities.gasMeterDamage',['Yes','No','Unknown'])}${seg('Damage to Gas Riser','utilities.gasRiserDamage',['Yes','No','Unknown'])}${field('Gas Notes','utilities.gasNotes','textarea')}`)}${card('Propane / LP Gas',`${seg('LP Present','utilities.lpTank',['Yes','No','Unknown'])}${field('LP Supplier','utilities.lpSupplier')}${field('LP Tank Size','utilities.lpTankSize')}${seg('Tank Orientation','utilities.lpOrientation',['Horizontal','Vertical','Buried','Unknown'])}${field('LP Serial Number','utilities.lpSerialNumber')}${field('Percent Product','utilities.lpPercentProduct','number')}${field('Number of Regulators','utilities.lpRegulators','number')}${seg('Tank Fire Damage','utilities.lpTankDamage',['Yes','No','Unknown'])}${field('LP Notes','utilities.lpNotes','textarea')}`)}${card('Additional Energy Systems',`${seg('Solar Panels','building.solarPanels',['Yes','No','Unknown'])}${seg('Generator','utilities.generator',['Yes','No','Unknown'])}${seg('Battery Storage','utilities.batteryStorage',['Yes','No','Unknown'])}${field('Energy System Notes','utilities.energySystemNotes','textarea')}`)}</div>`;
}
function electrical(){
  setTitle('Electrical');
  const panels = state.electrical?.panels || [];
  return `<div class="grid full">${card('Electrical Panel Documentation',`<div class="actions"><button class="btn blue" data-action="addPanel">Add Electrical Panel</button></div><div class="list">${panels.map((panel,i)=>electricalPanelCard(panel,i)).join('') || '<p class="muted">No electrical panels documented yet.</p>'}</div>`) }${card('General Electrical Notes',`${field('Electrical Notes','report.electricalNotes','textarea')}`)}</div>`;
}
function electricalPanelCard(panel,i){
  const circuits = Array.isArray(panel.circuits) ? panel.circuits : [];
  return `<div class="item"><div class="item-head"><strong>${esc(panel.location || `Electrical Panel ${i+1}`)}</strong><button class="pill" data-action="removePanel" data-index="${i}">Remove</button></div><div class="grid two">${dynField('electrical.panels',i,'location','Panel Location')}${dynField('electrical.panels',i,'manufacturer','Panel Manufacturer')}${dynField('electrical.panels',i,'type','Panel Type','select',{options:['','Main','Subpanel','Disconnect','Fuse Panel','Breaker Panel','Other']})}${dynField('electrical.panels',i,'mainBreakerSize','Main Breaker Size')}${dynField('electrical.panels',i,'fireImpingement','Fire Impingement','select',{options:['','Yes','No','Unknown']})}${dynField('electrical.panels',i,'heatImpingement','Heat Impingement','select',{options:['','Yes','No','Unknown']})}${dynField('electrical.panels',i,'arcFlash','Arc Flash','select',{options:['','Yes','No','Unknown']})}${dynField('electrical.panels',i,'notes','Panel Notes','textarea')}</div><h3>Circuits</h3><div class="actions"><button class="btn secondary" data-action="addCircuit" data-panel-index="${i}">Add Circuit</button></div><div class="list">${circuits.map((c,j)=>circuitCard(i,j)).join('') || '<p class="muted">No circuit entries yet.</p>'}</div></div>`;
}
function circuitCard(panelIndex,circuitIndex){
  const key = `electrical.panels.${panelIndex}.circuits`;
  return `<div class="item"><div class="item-head"><strong>Circuit ${circuitIndex+1}</strong><button class="pill" data-action="removeCircuit" data-panel-index="${panelIndex}" data-circuit-index="${circuitIndex}">Remove</button></div><div class="grid two">${dynField(key,circuitIndex,'number','Circuit Number')}${dynField(key,circuitIndex,'position','Side / Position')}${dynField(key,circuitIndex,'amps','Amps','number')}${dynField(key,circuitIndex,'status','Status','select',{options:['','On','Off','Tripped','Removed','Damaged','Unknown']})}${dynField(key,circuitIndex,'label','Circuit Label / Description','textarea')}</div></div>`;
}
...
function electricalPanelTemplate(){return {id:id(),location:'',manufacturer:'',type:'',mainBreakerSize:'',fireImpingement:'',heatImpingement:'',arcFlash:'',notes:'',circuits:[]};}
function circuitTemplate(){return {id:id(),number:'',position:'',amps:'',status:'',label:''};}
...
function handleAction(e){ const a=e.currentTarget.dataset.action, key=e.currentTarget.dataset.key; if(a==='goDashboard') render('dashboard'); if(a==='add'){ state[key].push(templates[key]()); save(); render(current); } if(a==='remove'){ state[key].splice(Number(e.currentTarget.dataset.index),1); save(); render(current); } if(a==='addPanel'){ state.electrical.panels.push(electricalPanelTemplate()); save(); render('electrical'); } if(a==='removePanel'){ state.electrical.panels.splice(Number(e.currentTarget.dataset.index),1); save(); render('electrical'); } if(a==='addCircuit'){ const panel = state.electrical.panels[Number(e.currentTarget.dataset.panelIndex)]; if(panel){ if(!Array.isArray(panel.circuits)) panel.circuits=[]; panel.circuits.push(circuitTemplate()); save(); render('electrical'); } } if(a==='removeCircuit'){ const panel = state.electrical.panels[Number(e.currentTarget.dataset.panelIndex)]; if(panel?.circuits){ panel.circuits.splice(Number(e.currentTarget.dataset.circuitIndex),1); save(); render('electrical'); } } if(a==='addInterview'){ state.interviews.push(interviewTemplate(e.currentTarget.dataset.guide)); save(); render('interviews'); } if(a==='exportFip') exportFip(false); if(a==='shareFip') exportFip(true); if(a==='importFip') $('#importFile').click(); if(a==='newCase') { if(confirm('Create a new blank case? Export the current case first if needed.')) caseEngine.create(); } if(a==='copyReport'){ navigator.clipboard?.writeText(reportText()); saveStatus.textContent='Report copied'; }}
