...
const blank = () => {
  ...
  initial:{caseNumber:'',incidentNumber:'',incidentType:'Structure',cause:'Undetermined',dateReported:'',timeReported:'',dateDispatched:'',timeDispatched:'',dateNotified:'',timeNotified:'',dateArrived:'',timeArrived:'',dateReleased:'',timeReleased:'',reportedDate:'',reportedTime:'',incidentAddress:'',city:'',state:'VA',jurisdiction:'',latitude:'',longitude:'',authority:'Exigency',consentName:'',dispatchTime:'',arrivalTime:'',sceneReleasedTime:'',otherLEO:[],leoOther:'',notes:''},
  caller:{name:'',phone:'',address:'',howReported:'',whatObserved:''},
  fireDept:{fireDepartment:'',incidentCommander:'',icUnit:'',alarms:'',firstEngine:'',engineOfficer:'',engineActivity:'',firstTruck:'',truckOfficer:'',truckActivity:'',fdNotes:''},
...
function migrate(data){
  ...
  normalizeInitialInformation(out);
...
function normalizeInitialInformation(target=state){
  const initial = target.initial || (target.initial = {});
  initial.dateReported = initial.dateReported || initial.reportedDate || '';
  initial.timeReported = initial.timeReported || initial.reportedTime || '';
  initial.timeDispatched = initial.timeDispatched || initial.dispatchTime || '';
  initial.timeArrived = initial.timeArrived || initial.arrivalTime || '';
  initial.timeReleased = initial.timeReleased || initial.sceneReleasedTime || '';
  initial.reportedDate = initial.dateReported || initial.reportedDate || '';
  initial.reportedTime = initial.timeReported || initial.reportedTime || '';
  initial.dispatchTime = initial.timeDispatched || initial.dispatchTime || '';
  initial.arrivalTime = initial.timeArrived || initial.arrivalTime || '';
  initial.sceneReleasedTime = initial.timeReleased || initial.sceneReleasedTime || '';
  if(initial.incidentType === 'Vehicle/Machinery') initial.incidentType = 'Vehicle / Machinery';
  if(!Array.isArray(initial.otherLEO)) initial.otherLEO = [];
}
function syncCaseFromLegacy(target=state){
  normalizeInitialInformation(target);
...
function dashboard(){
  ...
  return `<div class="grid full"><div class="kpi">${counts.map(c=>`<div class="card"><div class="muted">${c[0]}</div><div class="kpi-num">${c[1]}</div></div>`).join('')}</div>${card('Case Engine',`${field('Case Number','case.caseNumber')}${field('Incident Type','case.incidentType')}${field('Incident Address / Location','case.incidentAddress')}${field('Jurisdiction','case.jurisdiction')}${field('Primary Investigator','case.primaryInvestigator')}${field('Assisted By','case.assistedBy')}${field('Case Status','case.caseStatus','select',{options:['Draft','Active','Pending Review','Closed','Reopened','Archived']})}<div class="actions"><button class="btn blue" data-action="exportFip">Export .fip</button></div><div class="muted">Case ID: ${esc(c.id)} • Updated: ${esc((c.updatedAt||state.meta.updatedAt||'').replace('T',' ').slice(0,19))}</div>`)}${card('Dashboard Engine',`<div class="row two"><div><span class="badge">${esc(c.caseStatus||'Draft')}</span><p class="muted">Landing page for the active case after create/open/import. Modules continue to use the current PWA screens while Alpha 1.2 foundations are built.</p></div><div>${initialStatus}</div></div><div class="actions"><span class="badge">Offline-first</span><span class="badge">Autosave</span><span class="badge">.fip import/export</span><span class="badge">Shared case object</span></div>`)}${card('Approved Build Scope',`<ol class="muted"><li>Case Engine foundation</li><li>Dashboard Engine foundation</li><li>Initial Information foundation</li></ol><p class="muted">Timeline, Weather, Attachments, Building expansion, and full navigation reorganization are intentionally deferred.</p>`)}</div>`;
}
function initial(){ setTitle('Initial Information'); return `<div class="grid">${card('Date / Time',`${field('Date Reported','initial.dateReported','date')}${field('Time Reported','initial.timeReported','time')}${field('Date Dispatched','initial.dateDispatched','date')}${field('Time Dispatched','initial.timeDispatched','time')}${field('Date Notified','initial.dateNotified','date')}${field('Time Notified','initial.timeNotified','time')}${field('Date Arrived','initial.dateArrived','date')}${field('Time Arrived','initial.timeArrived','time')}${field('Date Released','initial.dateReleased','date')}${field('Time Released','initial.timeReleased','time')}`)}${card('Incident Information',`${field('Case Number','initial.caseNumber')}${field('Incident Number','initial.incidentNumber')}${field('Incident Address / Location','initial.incidentAddress')}${field('City','initial.city')}${field('State','initial.state')}${field('Jurisdiction','initial.jurisdiction')}${field('Latitude','initial.latitude','number')}${field('Longitude','initial.longitude','number')}${seg('Incident Type','initial.incidentType',['Structure','Vehicle / Machinery','Outside','Explosives'])}`)}${card('Authority of Scene Examination',`${seg('Authority','initial.authority',['Exigency','Consent','Search Warrant'])}${field('Consent Name','initial.consentName')}`)}${card('911 Caller Information',`${field('Name','caller.name')}${field('Phone Number','caller.phone','tel')}${field('Address','caller.address')}${field('How Reported','caller.howReported')}${field('What Caller Observed','caller.whatObserved','textarea')}`)}${card('Fire Suppression Information',`${field('Fire Department','fireDept.fireDepartment')}${field('Incident Commander','fireDept.incidentCommander')}${field('IC Unit ID','fireDept.icUnit')}${field('Number of Alarms','fireDept.alarms','number')}${field('First Arriving Engine Company','fireDept.firstEngine')}${field('Engine Officer','fireDept.engineOfficer')}${field('Engine Activity on Arrival','fireDept.engineActivity','textarea')}${field('First Arriving Truck / Specialty','fireDept.firstTruck')}${field('Truck / Specialty Officer','fireDept.truckOfficer')}${field('Truck / Specialty Activity on Arrival','fireDept.truckActivity','textarea')}`)}${card('Other Law Enforcement on Scene',`${checks('Agencies','initial.otherLEO',['LCSO','Leesburg PD','Purcellville PD','Middleburg PD','Other'])}${field('Other / Notes','initial.leoOther','textarea')}`)}${card('Assisted By',`${field('Assisted By','case.assistedBy','textarea')}`)}</div>`; }
